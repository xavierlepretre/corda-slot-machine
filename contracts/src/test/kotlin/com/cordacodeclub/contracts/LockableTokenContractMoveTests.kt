...
import com.cordacodeclub.contracts.LockableTokenContract.Commands.Move
...
class LockableTokenContractMoveTests {
    ...
    fun `Command must have inputs`() {
        Move(listOf(1), listOf(1))
        assertThrows<IllegalArgumentException> { Move(listOf(), listOf(1)) }
    }

    @Test
    fun `Command must have outputs`() {
        Move(listOf(1), listOf(1))
        assertThrows<IllegalArgumentException> { Move(listOf(1), listOf()) }
    }
    ...
    fun `Inputs must be lockable tokens`() {
        ledgerServices.transaction {
            ...
            command(casino.owningKey, Move(listOf(0), listOf(0)))
    ...
    fun `Outputs must have strictly positive amounts`() {
        ledgerServices.transaction {
            ...
            tweak {
                ...
                command(casino.owningKey, Move(listOf(0, 1), listOf(0, 1)))
                failsWith("The outputs must have positive amounts")
            }

            command(casino.owningKey, Move(listOf(0, 1), listOf(0)))
            verifies()
        }
    }
    ...
    fun `Inputs must have a single issuer`() {
        ledgerServices.transaction {
            ...
            output(LockableTokenContract.id, LockableTokenState(player, issuer, Amount(5, LockableTokenType)))
            command(casino.owningKey, Move(listOf(0, 1), listOf(0)))

            tweak {
                input(LockableTokenContract.id, LockableTokenState(player, player, Amount(4, LockableTokenType)))
    ...
    fun `Outputs must have a single issuer`() {
        ledgerServices.transaction {
            ...
            output(LockableTokenContract.id, LockableTokenState(player, issuer, Amount(2, LockableTokenType)))
            command(casino.owningKey, Move(listOf(0), listOf(0, 1)))

            tweak {
                output(LockableTokenContract.id, LockableTokenState(player, player, Amount(3, LockableTokenType)))
    ...
    @Test
    fun `Input holders must sign`() {
        ledgerServices.transaction {
            input(LockableTokenContract.id, LockableTokenState(casino, issuer, Amount(3, LockableTokenType)))
            input(LockableTokenContract.id, LockableTokenState(casino, issuer, Amount(5, LockableTokenType)))
            output(LockableTokenContract.id, LockableTokenState(player, issuer, Amount(8, LockableTokenType)))

            tweak {
                command(issuer.owningKey, Move(listOf(0, 1), listOf(0)))
    ...
    fun `All covered lockable token states must not overlap`() {
        ledgerServices.transaction {
            ...
            tweak {
                input(LockableTokenContract.id, LockableTokenState(casino, issuer, Amount(3, LockableTokenType)))
                command(casino.owningKey, Move(listOf(1), listOf(0)))
