...
function SlotMachinesBalanceChange(balance) {
  ...
  SlotMachines.balance = balance;
  slotMachines.forEach((it) => it.balanceChange(balance));
}
...
var SlotMachine = function ($elSlotsOuterContainer) {
  ...
  this.$elOutputBalance = $elSlotsOuterContainer.find(
    ".slot_machine_output_balance"
  );
  this.$elOutputDayWinnings = $elSlotsOuterContainer.find(
    ".slot_machine_output_day_winnings"
  );
  this.$elOutputLifetimeWinnings = $elSlotsOuterContainer.find(
    ".slot_machine_output_lifetime_winnings"
  );
  this.$elOutputBet = $elSlotsOuterContainer.find(".slot_machine_output_bet");
  this.$elOutputLastWin = $elSlotsOuterContainer.find(
    ".slot_machine_output_last_win"
  );
  this.$elBetIncreaseButton = $elSlotsOuterContainer.find(
    ".slot_machine_bet_increase_button"
  );
  this.$elBetDecreaseButton = $elSlotsOuterContainer.find(
    ".slot_machine_bet_decrease_button"
  );
  this.$elSpinButton = $elSlotsOuterContainer.find(".slot_machine_spin_button");
  this.$elSoundToggleButton = $elSlotsOuterContainer.find(
    ".slot_machine_sound_toggle_button"
  );

  // Bind events
  this.$elBetIncreaseButton.click(function () {
    _this.changeBet(+1);
  });
  this.$elBetDecreaseButton.click(function () {
    _this.changeBet(-1);
  });
  this.$elSpinButton.click(function () {
    _this.spin();
  });
  this.$elSoundToggleButton.click(function () {
    _this.toggleSound();
  });
...
SlotMachine.prototype.balanceChange = function (balance) {
  this.$elOutputBalance.html(SlotMachines.balance);
  if (SlotMachines.balance < this.minBet) {
    this.disableSpinButton();
  } else {
...
SlotMachine.prototype.changeBet = function (delta) {
  ...
  if (this.spinning) {
    return;
  } // don't do anything while spinning.
  ...
  this.$elPrizesList.find(".slot_machine_prize_payout").each(function () {
    ...
    $tdPayout.html(
      ($tdPayout.attr("data-payoutPrefix") || "") +
        parseFloat($tdPayout.attr("data-basePayout"), 10) * _this.curBet +
        ($tdPayout.attr("data-payoutSuffix") || "")
    );
  });
...
SlotMachine.prototype.showWonState = function (bWon, prize_id) {
  if (bWon) {
    ...
    this.$elPrizesList
      .find(".slot_machine_prize_row_" + prize_id)
      .addClass("won");
  } else {
...
SlotMachine.prototype.updateCountersToLatest = function (spinResult) {
  ...
  if (typeof spinResult.day_winnings != "undefined") {
    ...
    this.$elOutputDayWinnings.html(
      SlotMachines.config.winningsFormatPrefix +
        this.formatWinningsNumber(spinResult.day_winnings)
    );
  }
  if (typeof spinResult.lifetime_winnings != "undefined") {
    // TODO: Fix this formatting
    this.$elOutputLifetimeWinnings.html(
      SlotMachines.config.winningsFormatPrefix +
        this.formatWinningsNumber(spinResult.lifetime_winnings)
    );
  }
...
SlotMachine.prototype.spin = function () {
  ...
  if (this.$elSpinButton.hasClass("disabled")) {
    return false;
  }
  if (this.spinning) {
    return false;
  ...
  var fnFirstReelTimeout = function () {
    ...
    if (spinResult != null) {
      _this.stopReelsAndEndSpin(spinResult);
    } // If AJAX came back already, stop reels (this is the ideal case)
  };
  ...
  var fnAJAXRequestSuccess = function (data) {
    ...
    if (firstReelTimeoutHit == true) {
      _this.stopReelsAndEndSpin(spinResult);
    } // First reel should have stopped already, we are late
  };
...
SlotMachine.prototype.stopReelsAndEndSpin = function (spinResult) {
  ...
  window.setTimeout(function () {
    _this.stopReel(1, spinResult.reels[0]);
  }, baseTimeout);
  baseTimeout += SlotMachines.config.secondReelStopTime;
  window.setTimeout(function () {
    _this.stopReel(2, spinResult.reels[1]);
  }, baseTimeout);
  baseTimeout += SlotMachines.config.thirdReelStopTime;
  window.setTimeout(function () {
    _this.stopReel(3, spinResult.reels[2]);
  }, baseTimeout);
...
SlotMachine.prototype.handleServerError = function () {
  // TODO: See what scratchcards do here
  this.abortSpinAbruptly();
  this.$elSlotsOuterContainer
    .find(".slot_machine_failed_request_message")
    .show();
  return false;
};
...
SlotMachine.prototype.startSpinningReels = function () {
  ...
  this.spinReel(
    3,
    SlotMachines.config.secondReelStopTime +
      SlotMachines.config.thirdReelStopTime
  );
};
...
SlotMachine.prototype.spinReel = function (i, timeOffset) {
  ...
  var fnAnimation = function () {
    ...
    if (
      Date.now() <
      startTime + SlotMachines.config.reelSpeed1Time + timeOffset
    ) {
    ...
    if (curPos > 0) {
      curPos = -SlotMachines.config.stripHeight * 2;
    }
  };
...
SlotMachine.prototype.stopReel = function (i, outcome) {
  ...
  if (outcome != null) {
    ...
    var distanceBetweenIcons =
      SlotMachines.config.stripHeight / this.iconsPerReel;
    var finalPosition =
      -SlotMachines.config.stripHeight -
      (outcome - 1) * distanceBetweenIcons +
      SlotMachines.config.alignmentOffset;

    // Animation two: Elastic Easing
    $elReel
      .css({ top: finalPosition - SlotMachines.config.stripHeight })
      .animate(
        { top: finalPosition + SlotMachines.config.bounceHeight },
        SlotMachines.config.positioningTime,
        "linear",
        function () {
          $elReel.animate(
            { top: finalPosition },
            SlotMachines.config.bounceTime,
            "easeOutElastic"
          );
        }
      );
...
SlotMachine.prototype.incrementOutputCounters = function (spinResult) {
  ...
  var currentValues = {
    ...
    lifetime_winnings:
      spinResult.lifetime_winnings - spinResult.prize.payout_winnings,
  };
  ...
  var maxDelta = Math.max(
    spinResult.balance - currentValues.balance,
    spinResult.day_winnings - currentValues.day_winnings
  );
  ...
  var timerID = window.setInterval(function () {
    ...
    $.each(["balance", "day_winnings", "lifetime_winnings"], function (
      i,
      component
    ) {
      if (currentValues[component] < spinResult[component]) {
        currentValues[component] += 1;
        currentValues[component] = Math.min(
          currentValues[component],
          spinResult[component]
        ); // make sure we don't go over, useful for decimals.
        ...
        if (component != "balance") {
          formattedValue =
            SlotMachines.config.winningsFormatPrefix +
            _this.formatWinningsNumber(currentValues[component]);
        }
    ...
    if (!valueChanged) {
      // we're done updating all the counters
      window.clearInterval(timerID);
...
$(window).on("load", function () {
  ...
  $(".slot_machine_outer_container").each(function () {
    window.slotMachines.push(new SlotMachine($(this)));
  });
});
